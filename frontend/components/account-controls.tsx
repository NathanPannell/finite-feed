...
import styles from "@/components/settings-layout.module.css";

type Revision = { version: number; preference_statement: string; source: string; created_at: string };
export type Account = { id: string; email: string; display_name: string; telegram_connected: boolean; delivery_paused: boolean; delivery_status?: string; delivery_error?: string | null; onboarding_completed?: boolean };
...
export function AccountControls({ onAccountChange }: { onAccountChange?: (account: Account) => void }) {
  ...
  const loadAccount = useCallback(async () => {
    try { const response = await fetch("/api/personal/account", { cache: "no-store" }); if (!response.ok) throw new Error(); const nextAccount = await response.json() as Account; setAccount(nextAccount); onAccountChange?.(nextAccount); }
    catch { setError(true); setMessage("Your account could not load. Refresh your account to try again."); }
  }, [onAccountChange]);
  useEffect(() => { const timer = setTimeout(() => void loadAccount(), 0); return () => clearTimeout(timer); }, [loadAccount]);
  ...
  return <section className={styles.accountSheet} aria-labelledby="account-heading"><div><h2 id="account-heading">Your account</h2>{account ? <><p className={styles.accountIdentity}><strong>{account.display_name || account.email}</strong><br />{account.email}</p><button className="cancel-action" onClick={() => void signOut()} disabled={!!busy}>Sign out</button></> : <p role="status">{message || "Loading your account…"}</p>}</div><div><h2>Telegram delivery</h2><p>{account ? account.telegram_connected ? "Telegram is connected." : "Connect Telegram to receive your recommendations." : "Checking connection…"} {account?.delivery_paused ? "Delivery is paused." : ""}</p>{account?.delivery_status && <p>Last delivery: {account.delivery_status.replaceAll("_", " ")}{account.delivery_error ? `. ${account.delivery_error}` : ""}</p>}<div className={styles.accountActions}><button className="save-action" onClick={() => void action("link", "account/telegram-link")} disabled={!!busy || !account}>{busy === "link" ? "Preparing code…" : account?.telegram_connected ? "Get a new connection code" : "Connect manually"}</button><button className="cancel-action" onClick={() => void loadAccount()} disabled={!!busy}>Refresh account</button>{account?.telegram_connected && <button className="cancel-action" disabled={!!busy} onClick={() => void action("unlink", "account/telegram", "DELETE")}>Disconnect</button>}{account && <button className="cancel-action" disabled={!!busy} onClick={() => void action("pause", "account/delivery", "PUT", { paused: !account.delivery_paused })}>{account.delivery_paused ? "Resume delivery" : "Pause delivery"}</button>}</div>{telegramLink && <section className={styles.telegramSetup} aria-labelledby="telegram-setup-heading"><h3 id="telegram-setup-heading">Connect in Telegram</h3><ol><li>Open <strong>@{telegramBot}</strong> in Telegram.</li><li>Copy and send this six-digit code as a message.</li><li>Return here and refresh your account.</li></ol><div className={styles.telegramCode}><output aria-label="Your six-digit Telegram connection code">{telegramLink.code}</output><button className="cancel-action" onClick={() => void copyTelegramCode()}>{copiedCode ? "Copied" : "Copy code"}</button></div><p className="landing-note">This code expires {new Date(telegramLink.expires_at).toLocaleTimeString()}.</p><a className="telegram-deep-link" href={telegramLink.url} target="_blank" rel="noreferrer">Prefer a link? Open Telegram instead <span aria-hidden="true">↗</span></a></section>}</div>{message && <p className={`${error ? "signal-error" : "signal-notice"} ${styles.accountMessage}`} role={error ? "alert" : "status"}>{message}</p>}<details className={styles.accountData} onToggle={(event) => { if (event.currentTarget.open) void loadHistory(); }}><summary>Preference history</summary>{historyError ? <p role="alert">History could not load. <button onClick={() => void loadHistory()}>Try again</button></p> : history ? history.length ? history.map((revision) => <article key={revision.version}><h3>Version {revision.version}</h3><p>{revision.preference_statement}</p><p className="landing-note">{revision.source} · {new Date(revision.created_at).toLocaleDateString()}</p></article>) : <p>No preference changes yet. Edit interests above to get started.</p> : <p>Loading preference history…</p>}</details><details className={styles.accountData}><summary>Your data & account deletion</summary><p>Export your personal records at any time. Deleting your account removes its personal records and disconnects Telegram. Previously sent Telegram messages remain in your chat. <Link href="/privacy">Read about retention.</Link></p><div className={styles.accountActions}><button className="cancel-action" disabled={!!busy} onClick={() => void action("export", "account/export", "GET")}>Export account data</button><button className="cancel-action" disabled={!!busy} onClick={() => setDeleting(!deleting)}>Delete account…</button></div>{deleting && <form className={styles.deleteConfirmation} onSubmit={(event) => { event.preventDefault(); if (confirmation === "DELETE") void action("delete", "account", "DELETE", { confirmation }); }}><label htmlFor="delete-confirmation">This cannot be undone. Type DELETE to confirm.</label><input id="delete-confirmation" autoComplete="off" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /><button className="save-action" disabled={confirmation !== "DELETE" || !!busy}>{busy === "delete" ? "Deleting…" : "Permanently delete my account"}</button></form>}</details></section>;
}
