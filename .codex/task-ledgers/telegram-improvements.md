...
OAuth gate integration follow-up: first workflow run correctly failed closed because the real SDK returns a Neon /sign-in/social/init broker URL, while initial fixtures modeled a direct Google URL. Captured the actual deployed endpoint response without logging its token, verified the broker's 302 Google redirect and exact callback, and updated validator/tests to follow only that exact branch-specific hop. Root live end-to-end validator and 16 script tests pass. No app behavior or OAuth credentials changed.
