...
from backend.app.annotations import AnnotationConflict, clean_display_text, next_annotation, record_annotation
...
def _prepare_pair(conn: psycopg.Connection, suffix: int, *, last_served: str | None = None) -> UUID:
    ...
    conn.execute(
        ...
        VALUES (%s, %s, 'A practical description.', 'Test channel', NOW())
...
def test_queue_rotates_fairly_across_equally_unreviewed_pairs() -> None:
    with psycopg.connect(_database_url(), row_factory=dict_row) as conn:
