...
test("production batches exact service settings and preserves optional deletion", () => {
  ...
  configureVariables(plans, (args) => {
    calls.push(args);
    return args[1] === "list" ? JSON.stringify({ TELEGRAM_DEVELOPER_BOT_TOKEN: null, UNRELATED_SECRET: "untouched" }) : "";
  });
...
test("preview isolates database, strips production token and configures signing", () => {
  ...
  configureVariables(plans, (args) => { calls.push(args); return args[1] === "list" ? '{"TELEGRAM_PRODUCTION_BOT_TOKEN":null}' : ""; });
...
test("rejects unsafe targets and incomplete configuration before executing", () => {
  for (const key of ["RAILWAY_PROJECT_ID", "RAILWAY_API_SERVICE_ID", "RAILWAY_WORKER_SERVICE_ID", "YOUTUBE_API_KEY", "MATCH_LAB_COOKIE_SECRET", "TELEGRAM_DEVELOPER_BOT_TOKEN", "PREVIEW_DATABASE_URL", "PREVIEW_DATABASE_URL_UNPOOLED"]) {
